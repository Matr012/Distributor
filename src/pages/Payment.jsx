...
import { apiCreateUserCard, apiGetUserCards, apiCreateUserSubscription, apiCreateUserBilling } from '../utils/api';
...
export default function Payment() {
  ...
  const [selectedPlanId] = useState(() => localStorage.getItem('selectedPlanId') || '');
  ...
  const handleChange = (e) => {
    ...
    // Auto-format card number (space every 4 digits) + auto-detect type
    if (name === 'cardNumber') {
      value = value.replace(/\D/g, '');
      value = value.replace(/(\d{4})(?=\d)/g, '$1 ');
      const detectedType = detectCardType(value);
      setForm((prev) => ({ ...prev, cardNumber: value, cardType: detectedType }));
      setErrorMsg('');
      setErrorField('');
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    // Lejárati dátum ellenőrzés - ne lehessen lejárt kártyát megadni
    const [expMonth, expYear] = expiry.split('/');
    const fullYear = 2000 + parseInt(expYear, 10);
    const currentDate = new Date();
    const currentYear = currentDate.getFullYear();
    const currentMonth = currentDate.getMonth() + 1; // 0-indexed, ezért +1

    if (fullYear < currentYear || (fullYear === currentYear && parseInt(expMonth, 10) < currentMonth)) {
      setErrorMsg('A kártya lejárati dátuma nem lehet a múltban! Kérlek adj meg érvényes kártyát.');
      setErrorField('expiry');
      return;
    }
    ...
    // All valid — fizetés feldolgozása API-n keresztül
    setLoading(true);
    ...
    try {
      ...
      // Duplikált kártya ellenőrzés
      const existingCards = await apiGetUserCards();
      const userCards = existingCards.filter(c => c.userId === user.id);
      const isDuplicate = userCards.some(
        c => c.cardLast4 === numOnly.slice(-4) && c.cardExpMonth === parseInt(expMonth, 10) && c.cardExpYear === fullYear
      );

      if (!isDuplicate) {
        // Kártya mentése az API-n keresztül
        await apiCreateUserCard({
          id: 0,
          userId: user.id,
          cardHolderName: cardName.trim(),
          cardLast4: numOnly.slice(-4),
      ...
      // Előfizetés létrehozása az API-n keresztül
      const priceNum = parseInt(planPrice.replace(/\s/g, ''), 10) || 0;
      const planId = selectedPlanId ? parseInt(selectedPlanId, 10) : 1;

      await apiCreateUserSubscription({
        id: 0,
        userId: user.id,
        planId: planId,
      ...
      // Előfizetés státusz frissítése az AuthContext-ben
      await checkSubscription();
      setModalOpen(true);
    } catch {
      setLoading(false);
      setErrorMsg('Hiba történt a fizetés feldolgozása során.');
    }
  };
